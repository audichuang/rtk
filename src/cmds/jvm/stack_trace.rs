...
/// the leading `"at "` marker, the remainder starts with any of `app_packages`.
///
/// When `app_packages` is empty, every frame is considered an app frame
/// (framework collapsing disabled). Summary lines like `"\t... 42 more"` are
/// always framework artifacts.
fn is_application_frame(frame: &str, app_packages: &[String]) -> bool {
...
///
/// When `app_packages` is empty, all frames are considered app frames and no
...
fn add_frames(
    ...
    for frame in frames {
        ...
        if structural || is_application_frame(frame, app_packages) {
...
pub(crate) fn process(raw: &str, app_packages: &[String], max_lines: usize) -> Option<String> {
...
mod tests {
    ...
    fn is_app_frame_rejects_summary_dots() {
        ...
        assert!(!is_application_frame("\t... 42 more", &pkgs("com.example")));
    }

    #[test]
    fn is_app_frame_rejects_empty_or_whitespace() {
        assert!(!is_application_frame("", &pkgs("com.example")));
        assert!(!is_application_frame("   ", &pkgs("com.example")));
    }
    ...
    fn collect_root_cause(frames: &[&str], app_packages: &[String]) -> Vec<String> {
    ...
    fn root_cause_no_filter_keeps_all_frames() {
        ...
        let frame_refs: Vec<&str> = frames.iter().map(|s| s.as_str()).collect();
        let out = collect_root_cause(&frame_refs, &[]);
    ...
    fn root_cause_collapses_framework_as_before() {
        ...
        let out = collect_root_cause(&frames, &pkgs("com.example"));
        assert_eq!(
            out,
            vec![
                "\tat com.example.A.foo(A.java:1)",
    ...
    fn process_empty_returns_none() {
        assert!(process("", &pkgs("com.example"), 0).is_none());
        assert!(process("   \n  ", &pkgs("com.example"), 0).is_none());
    }

    #[test]
    fn process_single_segment_no_filter_returns_verbatim() {
        let trace = "java.lang.RuntimeException: boom\n\tat com.example.A.foo(A.java:1)";
        let out = process(trace, &[], 0).unwrap();
        assert_eq!(out, trace);
    }
    ...
    fn process_single_segment_collapses_framework() {
        ...
        let out = process(trace, &pkgs("com.example"), 0).unwrap();
        assert_eq!(
            out,
    ...
    fn collect_collapsed(frames: &[&str], app_packages: &[String]) -> Vec<String> {
        let frames: Vec<String> = frames.iter().map(|s| s.to_string()).collect();
        let mut out = Vec::new();
        add_frames(&mut out, &frames, app_packages, None);
        out
    }
    ...
    fn collapse_no_filter_keeps_everything() {
        ...
        let out = collect_collapsed(&frames, &[]);
        assert_eq!(out.len(), 3);
    }
    ...
    fn collapse_all_framework_yields_single_summary() {
        ...
        let out = collect_collapsed(&frames, &pkgs("com.example"));
    ...
    fn hard_cap_single_segment_simple_truncate() {
        ...
        // With app_package=&pkgs("com.example"), all frames are app frames.
    ...
    fn process_real_world_spring_fixture() {
        ...
        let out = process(trace, &pkgs("com.example"), 50).unwrap();
